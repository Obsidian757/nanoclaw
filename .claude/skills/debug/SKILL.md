...
|-----|----------|---------|
| **Main app logs** | `logs/damien.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/damien.error.log` | Host-side errors |
...
echo '{}' | container run -i \
  ...
  --entrypoint /bin/bash damien-agent:latest \
...
```bash
container run --rm --entrypoint /bin/bash damien-agent:latest -c 'ls -la /workspace/'
```
...
```bash
container run --rm --entrypoint /bin/bash damien-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
container run --rm -it --entrypoint /bin/bash damien-agent:latest
```
...
# Also clear the session ID from Damien's tracking
echo '{}' > data/sessions.json
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/damien.log | tail -5
...
```bash
echo "=== Checking Damien Container Setup ==="
...
container system status &>/dev/null && echo "OK" || echo "NOT RUNNING - Damien should auto-start it; check logs"

echo -e "\n4. Container image exists?"
echo '{}' | container run -i --entrypoint /bin/echo damien-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/damien.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
