---
name: add-gmail
description: Add Gmail integration to Damien. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail capabilities to Damien. It can be configured in two modes:
...
> How do you want to use Gmail with Damien?
>
> **Option 1: Tool Mode**
...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "Damien"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "Damien Gmail")
>    - Click **Create**
...
```bash
sleep 2 && launchctl list | grep damien
```
...
> - Create a Gmail label (e.g., "Damien")
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'Damien',  // the label name, address pattern, or prefix
  contextMode: 'thread',
...
```bash
sleep 3 && tail -20 logs/damien.log | grep -i email
```
...
```bash
tail -f logs/damien.log | grep -E "(email|Email)"
```
