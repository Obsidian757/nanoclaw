---
name: x-integration
description: X (Twitter) integration for Damien. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** Damien v1.0.0. Directory structure may change in future versions.
...
1. **Damien is installed and running** - WhatsApp connected, service active
...
npm run build
launchctl kickstart -k gui/$(id -u)/com.damien
# Verify: launchctl list | grep damien shows PID and exit code 0
```
...
| `DAMIEN_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/damien.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into Damien, make the following modifications:
...
All paths below are relative to project root (`DAMIEN_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep damien  # Should show PID and exit code 0 or -
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/damien.log | tail -20

# Script errors
grep -i "error\|failed" logs/damien.log | tail -20
```
...
container run damien-agent ls -la /app/src/skills/
```
