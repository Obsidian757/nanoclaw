...
Adds Parallel AI MCP integration to Damien for advanced web research capabilities.
...
- **Non-blocking Design** - Uses Damien scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Damien already set up and running
...
Add Parallel AI MCP servers after the damien server:
```typescript
const mcpServers: Record<string, any> = {
  damien: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__damien__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__damien__send_message
   4. Use mcp__damien__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | container run -i --entrypoint /bin/echo damien-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/damien.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.damien`
